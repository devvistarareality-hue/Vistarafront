...
export default function TowerFloorBuilder({ floors, setFloors, folder, existing = new Set(), onPersist, onGenerate, generating = false, note, industrial = false }) {
  ...
  const cell = { borderWidth: 1.5, borderColor: COLORS.border, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 7, fontSize: 13, color: TEXT, backgroundColor: COLORS.surface };
  ...
  return (
    <View>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 4 }}>
        ...
        <TouchableOpacity onPress={addBlock} style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 9, borderWidth: 1.5, borderColor: COLORS.blue2 }}>
        ...
        {!industrial && (
          <TouchableOpacity onPress={() => addFloorRow(blocksOf(floors)[0] || '')} style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 9, borderWidth: 1.5, borderColor: COLORS.blue2 }}>
      ...
      <View key={`blk-${blk}`}>
        {(blocksOf(floors).length > 1 || blk) ? (
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 10, marginBottom: 2, flexWrap: 'wrap' }}>
            ...
            <TouchableOpacity onPress={() => removeBlock(blk)} style={{ paddingHorizontal: 10, paddingVertical: 5, borderRadius: 7, borderWidth: 1.5, borderColor: COLORS.error2, backgroundColor: COLORS.errorBg }}>
      ...
      {rows.map(({ f, i }) => {
        ...
        return (
          <View key={i} style={{ borderWidth: 1.5, borderColor: COLORS.border, borderRadius: 16, padding: 12, marginTop: 12 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
              <View style={{ flex: 1 }}><Text style={lbl}>Prefix</Text>
                <TextInput value={f.prefix || ''} placeholder="e.g. Shop" placeholderTextColor={COLORS.textTertiary} onChangeText={(v) => edit(i, { prefix: v })} onBlur={() => persist(floors)} style={cell} /></View>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 9, flexWrap: 'wrap' }}>
              <Text style={{ fontSize: 12, color: units.length ? COLORS.text2 : COLORS.error, flex: 1 }}>
                {units.length
      ...
      {planned.length > 0 && (
        <View style={{ borderTopWidth: 1, borderTopColor: COLORS.surfaceAlt, marginTop: 14, paddingTop: 12 }}>
          <Text style={{ fontSize: 13, color: COLORS.text2, marginBottom: 10 }}>
            ...
            <Text style={{ fontWeight: '800', color: toCreate.length ? COLORS.warning : COLORS.success }}>
          ...
          {onGenerate ? (
            <TouchableOpacity onPress={() => onGenerate(toCreate)} disabled={generating || !toCreate.length}
              style={{ backgroundColor: toCreate.length && !generating ? NAVY : COLORS.blue2, borderRadius: 14, paddingVertical: 12, alignItems: 'center' }}>
