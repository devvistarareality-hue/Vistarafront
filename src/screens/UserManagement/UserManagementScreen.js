...
import { fetchUsers, deleteUser } from '../../redux/actions/userManagementActions';
...
function UserCard({ user, onDelete, onEdit }) {
  ...
  const avatarBg = ROLE_AVATAR_COLOR[user.role] || '#8492A6';
  const badge    = ROLE_BADGE_STYLE[user.role]  || { bg: '#F0F0F0', text: '#555' };

  const confirmDelete = () =>
    Alert.alert(
      'Remove User',
      `Remove ${user.name} from the company? They will lose all access.`,
      [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Remove', style: 'destructive', onPress: () => onDelete(user.id) },
      ],
    );
  ...
  return (
    <TouchableOpacity style={s.card} activeOpacity={0.85} onPress={onEdit}>
      ...
      <View style={s.cardBody}>
        ...
        <View style={s.tagRow}>
          <View style={[s.roleBadge, { backgroundColor: badge.bg }]}>
            <Text style={[s.roleBadgeText, { color: badge.text }]}>{user.role}</Text>
          </View>
          {user.is_manager && (
      ...
      <View style={{ gap: 8, alignItems: 'center' }}>
        <TouchableOpacity style={s.editBtn} onPress={onEdit} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
          <Ionicons name="pencil-outline" size={16} color={COLORS.secondary} />
        </TouchableOpacity>
        <TouchableOpacity style={s.deleteBtn} onPress={confirmDelete} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
...
export default function UserManagementScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={s.screen} edges={['top']}>
      ...
      {/* ── Role filter tabs (fixed-height wrapper prevents vertical stretch) ── */}
