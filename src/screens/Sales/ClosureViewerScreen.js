...
function UnitModal({ plot, project, sv, user, onClose, onClosed }) {
  ...
  async function recordClosure() {
    if (!sv) { setErr('Start from Record Closure on a site visit.'); return; }
    ...
    try {
      ...
      if (res.ok) {
        ...
        // NOTE: booked unit intentionally NOT auto-marked Sold yet (pending decision).
        onClosed();
      } else {
            ...
            {!sv ? (
              ...
              <View style={{ borderTopWidth: 1, borderTopColor: COLORS.surfaceAlt, paddingTop: 14 }}>
                ...
                <View style={{ flexDirection: 'row', gap: 10 }}>
                  <View style={{ flex: 1 }}><Text style={lblS}>Unit No.</Text>
                    <TextInput value={cForm.unit_no} onChangeText={t => setCForm({ ...cForm, unit_no: t })} style={inpS} /></View>
                  <View style={{ flex: 1 }}><Text style={lblS}>Unit Type</Text>
                    <TextInput value={cForm.unit_type} onChangeText={t => setCForm({ ...cForm, unit_type: t })} style={inpS} /></View>
                </View>
                <View style={{ flexDirection: 'row', gap: 10 }}>
                ...
                <TextInput value={cForm.remarks} onChangeText={t => setCForm({ ...cForm, remarks: t })} multiline placeholder="Notes…" style={[inpS, { height: 70, textAlignVertical: 'top' }]} />
                {!!err && <Text style={{ color: COLORS.error, fontSize: 12, marginTop: 8 }}>{err}</Text>}
                <TouchableOpacity onPress={recordClosure} disabled={saving}
                  ...
                  <Text style={{ color: COLORS.white, fontWeight: '800', fontSize: 15 }}>{saving ? 'Saving…' : 'Save Closure'}</Text>
                </TouchableOpacity>
              </View>
            )}
          </ScrollView>
