...
export default function SalesReportsScreen({ navigation }) {
  ...
  const TELECALLER_CARDS = [
    ...
    { group: 'Conversions', label: 'Pending from Accounts', value: _accPending, color: COLORS.warning, bg: COLORS.warningBg, target: 'ClosureProjects', params: { initialView: 'mybookings', initialTab: 'accounts', initialScope: 'visible' } },
  ];
  const STM_CARDS = [
  ...
  const STAT_CARDS = isStmView ? STM_CARDS
    : (isAdmin || isManager)
      ? [...TELECALLER_CARDS.slice(0, 2), UNASSIGNED_CARD, ...TELECALLER_CARDS.slice(2)].filter((c) => !c.tcOnly)
      : TELECALLER_CARDS;
