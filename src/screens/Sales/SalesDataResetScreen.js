...
export default function SalesDataResetScreen({ navigation }) {
  ...
  function confirmReset() {
    ...
    Alert.alert(
      ...
      'This permanently deletes all trial leads, bookings, closures and related data for this company. This cannot be undone.',
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: BG }} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        <View style={[CARD, { padding: 16, marginBottom: 14 }]}>
          <Text style={{ fontSize: 11, fontWeight: '800', letterSpacing: 0.5, color: MUTED, textTransform: 'uppercase', marginBottom: 12 }}>What will be cleared</Text>
        ...
        <View style={{ backgroundColor: '#FEF2F2', borderWidth: 1.5, borderColor: RED, borderRadius: 14, padding: 16 }}>
          ...
          <TouchableOpacity onPress={confirmReset} disabled={confirmText !== 'DELETE' || busy}
            style={{ backgroundColor: (confirmText === 'DELETE' && !busy) ? RED : '#F3B4B4', borderRadius: 10, paddingVertical: 13, alignItems: 'center' }}>
            {busy ? <ActivityIndicator color="#fff" /> : <Text style={{ color: '#fff', fontWeight: '800', fontSize: 14 }}>Permanently delete {total} records</Text>}
          </TouchableOpacity>
